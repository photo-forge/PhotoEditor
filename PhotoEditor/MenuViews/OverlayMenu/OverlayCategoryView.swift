...
public protocol OverlayCategoryViewDelegate {
    func overlayCategoryView_didSelectAt(overlayCategoryName:String)
}

class OverlayCategoryView: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
    
    var delegate:OverlayCategoryViewDelegate!
    ...
    override init(frame: CGRect) {
        ...
        let categoryImages = overlayInfoDic?["Categories"] as? [AnyHashable]
        ...
        collectionView.register(OverlayMenuCategoryCell.self, forCellWithReuseIdentifier: "CategoryCell")
...
//        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "CategoryCell", for: indexPath) as! OverlayMenuCategoryCell
        let catName = categoryNames[indexPath.item] 
        cell.imageView.image = CommonMethods.ins.uiImageWithName(named: catName)
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if (delegate != nil) {
            delegate.overlayCategoryView_didSelectAt(overlayCategoryName:categoryNames[indexPath.row])
        }
    }
    
}

class OverlayMenuCategoryCell: UICollectionViewCell {
